...
import MonthlyTicket from '../../../Layout/ticket/Daily/PocketsRockets';
...
const MonthlyGame: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSoldTickets = async () => {
      try {
        const monthlyDocRef = doc(db, "Daily", "Pockets Rockets");
  ...
  const renderNumbers = () => {
    ...
    return (
      <div className='bg-green-500 h-[620px] w-[340px] rounded-xl flex flex-col items-center justify-center p-2'>
        <h1 className='font-extrabold text-2xl text-white m-10'>Pockets Rockets</h1>
  ...
  return (
    <>
      <div className='sm:overflow-hidden w-full flex flex-col items-center justify-center mb-10'>
        <Header />
        <div className='mt-20'>
          <LotteryInfo gameName={"Pockets Rockets"} />
        </div>
