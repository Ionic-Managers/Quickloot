...
import MonthlyTicket from '../../../Layout/ticket/Daily/CashWave';
...
const MonthlyGame: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSoldTickets = async () => {
      try {
        const monthlyDocRef = doc(db, "Daily", "Cash Wave");
  ...
  const renderNumbers = () => {
    ...
    return (
      <div className='bg-green-500 h-[620px] w-[340px] rounded-xl flex flex-col items-center justify-center p-2'>
        <h1 className='font-extrabold text-2xl text-white m-10'>Ball Buster</h1>
  ...
  return (
    <>
      <div className='sm:overflow-hidden w-full flex flex-col items-center justify-center mb-10'>
        <Header />
        <div className='mt-20'>
          <LotteryInfo gameName={"Cash Wave"} />
        </div>
