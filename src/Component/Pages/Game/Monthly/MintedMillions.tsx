...
import MonthlyTicket from '../../../Layout/ticket/Monthly/MintedMillions';
...
const MonthlyGame: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSoldTickets = async () => {
      const monthlyDocRef = doc(db, "Monthly", "Minted Millions");
  ...
  const renderNumbers = () => {
    ...
    return (
      <div className='bg-green-500 h-[620px] w-[340px] rounded-xl flex flex-col items-center justify-center p-2'>
        <h1 className='font-extrabold text-2xl text-white m-10'>Minted Millions</h1>
  ...
  return (
    <>
      <div className='sm:overflow-hidden w-full flex flex-col items-center justify-center mb-10'>
        <Header />
        <div className='mt-20'>
          <LotteryInfo gameName={"Minted Millions"} />
        </div>
