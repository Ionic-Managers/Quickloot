...
import MonthlyTicket from '../../../Layout/ticket/Monthly/RainbowRise';
    ...
    return filteredCodes.map((code, index) => {
      ...
      return (
        <button onClick={() => {
          ...
          RR{isSold && <span className="absolute top-0 right-0 bg-red-500 text-white py-1 px-2 rounded-full text-xs">Sold</span>}
          {code}
        </button>
      );
    });
  ...
  const renderNumbers = () => {
    ...
    return (
      <div className='bg-green-500 h-[620px] w-[340px] rounded-xl flex flex-col items-center justify-center p-2'>
        <h1 className='font-extrabold text-2xl text-white m-10'>RainbowRise</h1>
  ...
  return (
    <>
      <div className='sm:overflow-hidden w-full flex flex-col items-center justify-center mb-10'>
        <Header />
        <div className='mt-20'>
          <LotteryInfo gameName={"Rainbow Rise"} />
        </div>
