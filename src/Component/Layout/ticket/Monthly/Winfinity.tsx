...
import WinfinityTicket from '../../../../../public/TicketImages/MonthlyTicket/Winfinity.jpg';
  ...
  const downloadTicket = async () => {
    if(balance >= 999){
      try {
        ...
        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 999;
        }
    ...
    <>
      <div className='tref w-[400px] absolute left-0 sm:w-1/2  flex flex-col items-center mt-[70px] ' ref={ticketRef}>
        <div className='w-full h-[130px]'
          style={{
            backgroundImage: `url(${WinfinityTicket})`,
            backgroundRepeat: "no-repeat",
            backgroundSize: "cover",
          }}>
          <p className="text-[7px] font-black relative top-[119px] left-20">{selectedNumbers.join(' , ')}</p>
          <p className="text-[8px] relative top-[108px] text-center left-20">W{ticketCode}</p>
          <p className="text-[8px] absolute left-[319px] top-6 text-white">W{ticketCode}</p>
