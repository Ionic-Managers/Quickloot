...
import goldenboxTicket from '../../../../assets/Goldenbox.jpg';
  ...
  const downloadTicket = async () => {
    if (balance >= 610) {
      try {
        ...
        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 610;
        }
    ...
    <>
      <div className='tref w-[400px] absolute left-0 sm:w-1/2  flex flex-col items-center mt-[70px] ' ref={ticketRef}>
        <div className='w-full h-[130px]'
          ...
          <p className="text-[8px] relative top-[108px] text-center left-20">GB{ticketCode}</p>
          <p className="text-[8px] absolute left-[315px] top-6 text-white">GB{ticketCode}</p>
      ...
      <div className="absolute top-72 left-28">
        <button onClick={downloadTicket} className="bg-yellow-300 hover:bg-yellow-200 text-blue-800 font-bold py-2 px-4 rounded">
          Download Ticket
        </button>
      </div>
    </>
