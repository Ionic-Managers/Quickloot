...
import MintedMillionsTicket from '../../../../../public/TicketImages/MonthlyTicket/MintedMillions.jpg';
  ...
  const downloadTicket = async () => {
    if(balance >= 1499){
      try {
        ...
        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 1499;
        }
    ...
    <>
      ...
      <div className='tref w-[400px] absolute left-0 sm:w-1/2  flex flex-col items-center mt-[70px]' ref={ticketRef}>
        ...
        <div className='w-full h-[130px]'
          style={{
            backgroundImage: `url(${MintedMillionsTicket})`,
            backgroundRepeat: "no-repeat",
            backgroundSize: "cover",
          }}>
          <p className="text-[7px] font-black relative top-[119px] left-20">{selectedNumbers.join(' , ')}</p>
          <p className="text-[8px] relative top-[108px] text-center left-20">MM{ticketCode}</p>
          <p className="text-[8px] absolute left-[315px] top-7 text-white">MM{ticketCode}</p>
