...
import Bet2nite from '../../../../../public/TicketImages/DailyTicket/Bet2nite.jpg';
  ...
  const downloadTicket = async () => {
    if (balance >= 199) {
      try {
        ...
        const buyersListRef = doc(db, "Daily", "Bet2nite");
        const buyersDoc = await getDoc(buyersListRef);
        let newTotal = 199;

        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 199;
        }
    ...
    <>
      <div className='tref w-[400px] absolute left-0 sm:ml-72 sm:w-[950px]  sm:-mt-1 flex flex-col items-center mt-[70px]' ref={ticketRef}>
        <div className='w-full h-[130px] sm:h-[310px]'
          ...
          <p className="text-[8px] relative top-[107px] text-center left-20 sm:text-lg sm:relative sm:top-[252px] sm:left-[148px] sm:font-bold ">Bn{ticketCode}</p>
          <p className="text-[8px] absolute left-[317px] top-6  text-white sm:text-lg sm:relative sm:top-1 sm:left-[770px]">Bn{ticketCode}</p>
