...
import CashCrazeTicket from '../../../../../public/TicketImages/DailyTicket/Cashcraze.jpg';
import firebase from 'firebase/compat/app';
import 'firebase/compat/firestore';

interface PowerSwipeProps {
  selectedNumbers: number[];
  ticketCode: number;
}

const PowerSwipe: React.FC<PowerSwipeProps> = ({ selectedNumbers, ticketCode }) => {
  ...
  const downloadTicket = async () => {
    if (balance >= 89) {
      try {
        ...
        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 89;
        }
    ...
    <>
      <div className='tref w-[400px] absolute left-0 sm:w-1/2  flex flex-col items-center mt-[60px] ' ref={ticketRef}>
        <div className='w-full h-[130px]'
          style={{
            backgroundImage: `url(${CashCrazeTicket})`,
            backgroundRepeat: "no-repeat",
            backgroundSize: "cover",
          }}>
          <p className="text-[7px] font-black relative top-[119px] left-20">{selectedNumbers.join(' , ')}</p>
          <p className="text-[8px] relative top-[108px] text-center left-20">CC{ticketCode}</p>
          <p className="text-[8px] absolute left-[315px] top-6 text-white">CC{ticketCode}</p>
