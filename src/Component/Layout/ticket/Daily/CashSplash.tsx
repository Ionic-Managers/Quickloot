...
import CashSplashTicket from '../../../../../public/TicketImages/DailyTicket/Cashsplash.jpg';
  ...
  const downloadTicket = async () => {
    if (balance >= 80) {
      try {
        ...
        const buyersListRef = doc(db, "Daily", "Cash Splash");
        const buyersDoc = await getDoc(buyersListRef);
        let newTotal = 80;

        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 80;
        }
    ...
    <>
      <div className='tref w-[400px] absolute left-0 sm:ml-72 sm:w-[950px]  sm:-mt-1 flex flex-col items-center mt-[70px]' ref={ticketRef}>
        <div className='w-full h-[130px] sm:h-[310px]'
          style={{
            backgroundImage: `url(${CashSplashTicket})`,
         ...
         <p className="text-[7px] font-black relative top-[119px] left-20 sm:text-xl sm:relative sm:top-[280px] sm:left-48">{selectedNumbers.join(' , ')}</p>
          <p className="text-[8px] relative top-[107px] text-center left-20 sm:text-lg sm:relative sm:top-[252px] sm:left-[148px] sm:font-bold ">CS{ticketCode}</p>
          <p className="text-[8px] absolute left-[317px] top-6  text-white sm:text-lg sm:relative sm:top-1 sm:left-[770px]">CS{ticketCode}</p>
