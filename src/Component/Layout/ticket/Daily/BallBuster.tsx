...
import BallbusterTicket from '../../../../../public/TicketImages/DailyTicket/Ballbuster.jpg';
  ...
  const downloadTicket = async () => {
    if (balance >= 149) {
      try {
        ...
        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 149;
        }
  ...
  return (
    <>
      ...
      <div className='w-full h-[130px]'
          style={{
            backgroundImage: `url(${BallbusterTicket})`,
            backgroundRepeat: "no-repeat",
            backgroundSize: "cover",
          }}>
          <p className="text-[7px] font-black relative top-[119px] left-20">{selectedNumbers.join(' , ')}</p>
          <p className="text-[8px] relative top-[108px] text-center left-20">BB{ticketCode}</p>
          <p className="text-[8px] absolute left-[315px] top-6 text-white">BB{ticketCode}</p>
