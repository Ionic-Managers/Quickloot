...
import GoldeRushTicket from '../../../../../public/TicketImages/DailyTicket/GoldRush.jpg';
  ...
  const downloadTicket = async () => {
    if (balance >= 69) {
      try {
        ...
        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 69;
        }
    ...
    <>
      <div className='tref w-[400px] absolute left-0 sm:ml-72 sm:w-[950px]  sm:-mt-1 flex flex-col items-center mt-[70px]' ref={ticketRef}>
        <div className='w-full h-[130px] sm:h-[310px]'
          style={{
            backgroundImage: `url(${GoldeRushTicket})`,
          ...
          <p className="text-[8px] relative top-[107px] text-center left-20 sm:text-lg sm:relative sm:top-[252px] sm:left-[148px] sm:font-bold ">GR{ticketCode}</p>
          <p className="text-[8px] absolute left-[317px] top-6  text-white sm:text-lg sm:relative sm:top-1 sm:left-[770px]">GR{ticketCode}</p>
