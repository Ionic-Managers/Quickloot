...
import TurkeyTicket from '../../../../../public/TicketImages/WeeklyTicket/Turkeyshoot.jpg';
  ...
  const downloadTicket = async () => {
    if (balance >= 209) {
      try {
        ...
        const buyersListRef = doc(db, "Weekly", "Turkey Shoot");
        const buyersDoc = await getDoc(buyersListRef);
        let newTotal = 209;

        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 209;
        }
    ...
    <>
      <div className='tref w-[400px] absolute left-0 sm:ml-72 sm:w-[950px]  sm:-mt-1 flex flex-col items-center mt-[70px]' ref={ticketRef}>
        <div className='w-full h-[130px] sm:h-[310px]'
          style={{
            backgroundImage: `url(${TurkeyTicket})`,
