...
import SyndicateTicket from '../../../../../public/TicketImages/WeeklyTicket/Syndicatestar.jpg';
  ...
  const downloadTicket = async () => {
    if (balance >= 125) {
      try {
        ...
        const buyersListRef = doc(db, "Weekly", "Syndicate Star");
        const buyersDoc = await getDoc(buyersListRef);
        let newTotal = 125;

        if (buyersDoc.exists()) {
          newTotal = (buyersDoc.data().total || 0) + 125;
        }
    ...
    <>
      <div className='tref w-[400px] absolute left-0 sm:ml-72 sm:w-[950px]  sm:-mt-1 flex flex-col items-center mt-[70px]' ref={ticketRef}>
        <div className='w-full h-[130px] sm:h-[310px]'
          style={{
            backgroundImage: `url(${SyndicateTicket})`,
          ...
          <p className="text-[8px] relative top-[107px] text-center left-20 sm:text-lg sm:relative sm:top-[252px] sm:left-[148px] sm:font-bold ">SS{ticketCode}</p>
          <p className="text-[8px] absolute left-[317px] top-6  text-white sm:text-lg sm:relative sm:top-1 sm:left-[770px]">SS{ticketCode}</p>
